...
def register(mcp: object) -> None:
    ...
    def harness_session_create(workdir: str = "") -> str:
        ...
        if not result.session_id:
            return json.dumps({"error": "Failed to create session", "text": result.text})
        return json.dumps({"session_id": result.session_id})

    @mcp.tool()
    def harness_session_prompt(session_id: str, prompt: str, workdir: str = "", timeout: int = 300) -> str:
        ...
        args = [cmd, "run", "--format", "json", "--continue", "--session", session_id, prompt]
        ...
        try:
            ...
            return json.dumps({"error": f"Timed out after {timeout}s", "session_id": session_id})
        except FileNotFoundError:
            return json.dumps({"error": "opencode CLI not found", "session_id": session_id})
        ...
        return json.dumps({
            ...
            "tool_calls": [
                {"tool": t.tool, "status": t.status, "title": t.title, "duration_ms": t.duration_ms}
    ...
    def harness_session_list() -> str:
        ...
        cmd = find_opencode()
        try:
            result = subprocess.run([cmd, "session", "list"], capture_output=True, text=True, timeout=15)
        except Exception as e:
            return f"Error: {e}"
    ...
    def harness_session_fork(session_id: str, workdir: str = "") -> str:
        ...
        cmd = find_opencode()
        cwd = safe_workdir(workdir)
        args = [cmd, "run", "--format", "json", "--fork", "--session", session_id, "fork session"]

        env = os.environ.copy()
        env["OPENCODE_DISABLE_AUTOCOMPACT"] = "1"

        try:
            result = subprocess.run(args, cwd=cwd, capture_output=True, text=True, timeout=30, env=env)
        except Exception as e:
